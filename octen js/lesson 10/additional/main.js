...
//     console.log(obj.id);
//     console.log(obj.clientHeight, obj.clientWidth);
//
// })
...
function filter (arr) {
    ...
    if (status.checked){
        arr = arr.forEach((e) =>{ return e.status===false})
    }
    if (year.checked){
        arr = arr.filter(e => e.year >= 29)
    }
    if (city.checked){
        arr = arr.filter(e => e.city === 'Kyiv')
    }
    console.log(status.checked===true);
    return arr;
}
